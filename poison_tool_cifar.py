...
def get_train_loader(args):
    ...
    if (args.dataset == 'CIFAR10'):
        trainset = datasets.CIFAR10(root='/home/shunjie/codes/robust_training_against_backdoor/ours_new_box/DivideMix-master/CIFAR10', train=True, download=True)
    else:
...
def get_test_loader(args):
    ...
    testset = datasets.CIFAR10(root='/home/shunjie/codes/robust_training_against_backdoor/ours_new_box/DivideMix-master/CIFAR10', train=False, download=True)

    test_data_clean = DatasetBD(args, full_dataset=testset, poison_rate=0, transform=tf_test, mode='test')
    test_clean_loader = DataLoader(dataset=test_data_clean, batch_size=args.batch_size*2, shuffle=False, num_workers=4)
    
    test_data_bad = DatasetBD(args, full_dataset=testset, poison_rate=1, transform=tf_test, mode='test')
    test_bad_loader = DataLoader(dataset=test_data_bad, batch_size=args.batch_size*2, shuffle=False, num_workers=4)

    # testset = datasets.CIFAR10(root='/home/shunjie/codes/robust_training_against_backdoor/ours_new_box/DivideMix-master/CIFAR10', train=False, download=True, transform=tf_test)
    # test_clean_loader = DataLoader(dataset=testset,batch_size=args.batch_size*2, shuffle=False,num_workers=4)
...
def get_backdoor_loader(args):
    ...
    train_data_bad = DatasetBD(args, full_dataset=trainset, poison_rate=args.poison_rate, transform=tf_train, mode='train')
...
class DatasetBD(Dataset):
    def __init__(self, args, full_dataset, poison_rate, transform=None, mode="train", distance=1):
        self.args = args
        self.dataset, self.poison_indices = self.addTrigger(full_dataset, args.target_label, poison_rate, mode, distance, args.trig_w, args.trig_h, args.trigger_type, args.target_type)
    ...
    def __getitem__(self, index):
        ...
        label = self.dataset[index][1]
        # gound_truth = self.dataset[index][2]
    ...
    def addTrigger(self, dataset, target_label, poison_rate, mode, distance, trig_w, trig_h, trigger_type, target_type):
        print("Generating " + mode + " bad Imgs")
        
        if not os.path.exists(self.args.resume_path):
            os.makedirs(self.args.resume_path)
                
        if mode == 'train' and os.path.exists(f'{self.args.resume_path}/perm_dataset={self.args.dataset}_id={self.args.id}_trig_type={self.args.trigger_type}_target_label={self.args.target_label}_target_type={self.args.target_type}_trig_w={self.args.trig_w}_trig_h={self.args.trig_h}_pr={poison_rate}.npy'):
            perm = np.load(f'{self.args.resume_path}/perm_dataset={self.args.dataset}_id={self.args.id}_trig_type={self.args.trigger_type}_target_label={self.args.target_label}_target_type={self.args.target_type}_trig_w={self.args.trig_w}_trig_h={self.args.trig_h}_pr={poison_rate}.npy')
        else:
            perm = np.random.permutation(len(dataset))[0: int(len(dataset) * poison_rate)]
            np.save(f'{self.args.resume_path}/perm_dataset={self.args.dataset}_id={self.args.id}_trig_type={self.args.trigger_type}_target_label={self.args.target_label}_target_type={self.args.target_type}_trig_w={self.args.trig_w}_trig_h={self.args.trig_h}_pr={poison_rate}.npy',perm)
            ...
            if target_type == 'all2one':
                ...
                if mode == 'train':
                    ...
                    if i in perm:
                        ...
                        # change target
                        dataset_.append((img, target_label, data[1]))
                        cnt += 1
                    else:
                        dataset_.append((img, data[1], data[1]))

                else:
                    if data[1] == target_label and poison_rate == 1:
                        continue
    ...
    def _blendTrigger(self, img, pattern=None, weight=None):
        blend_mask = cv2.imread('/home/shunjie/codes/robust_training_against_backdoor/ours_new_box/DivideMix-master/triggers_image/hellokitty_32.png')
        img_ = np.clip((0.8*img + 0.2*blend_mask).astype('uint8'), 0, 255)
        return img_
    ...
    def _signalTrigger(self, img, width, height, distance, trig_w, trig_h):
        alpha = 0.2
        # load signal mask
        signal_mask = np.load('/home/shunjie/codes/robust_training_against_backdoor/ours_new_box/DivideMix-master/trigger/signal_cifar10_mask.npy')
    ...
    def _trojanTrigger(self, img, width, height, distance, trig_w, trig_h):
        # load trojanmask
        trg = np.load('/home/shunjie/codes/robust_training_against_backdoor/ours_new_box/DivideMix-master/trigger/best_square_trigger_cifar10.npz')['x']
        # trg.shape: (3, 32, 32)
    ...
    def _CLTrigger(self, img, mode='train', p_index=0):
        ...
        # Add triger
        if mode == 'train':
            trigger = np.load('/home/shunjie/codes/robust_training_against_backdoor/ours_new_box/DivideMix-master/trigger/two_600.npy')[p_index]
            img = img / 255
    ...
    def _dynamicTrigger(self, img, mode='Train'):
        ...
        ckpt_path = '/home/shunjie/codes/robust_training_against_backdoor/ours_new_box/DivideMix-master/all2one_cifar10_ckpt.pth.tar'
...
#     poison_set = train_data_bad = DatasetBD(opt=opt, full_dataset=split_set, poison_rate=0.1, transform=tf_train, mode='train')
